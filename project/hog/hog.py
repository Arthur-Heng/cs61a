...
def hefty_hogs(player_score, opponent_score):
    ...
    while opponent_score:
        last_num = opponent_score % 10
        opponent_score = opponent_score // 10
        player_score = digit_fn(last_num)(player_score)
    player_score = (player_score+30) % 30
    return player_score
    # END PROBLEM 2
...
def hog_pile(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    score = lambda x,y: x % 10 if x%10==y%10 else 0
    return score(player_score,opponent_score)
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score1 < goal and score0 < goal:
        if who==0:
            ...
            score0 += take_turn(strategy,score0,score1,dice=dice,goal=goal)
            ...
            score1 += take_turn(strategy,score1,score0,dice=dice,goal=goal)
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if leader != last_leader:
        return leader , "Player "+str(leader)+" takes the lead by "+str(abs(score0-score1))
    else:
        return leader,None
    # END PROBLEM 6
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if hog_pile(score+hefty_hogs(score,opponent_score),opponent_score):
        return 0
    else:
